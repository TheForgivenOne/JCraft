...
export default function HomePage() {
  ...
  useEffect(() => {
    if (memoizedAllProducts && memoizedAllProducts.length > 0) {
      // For now, just take the first few products as featured
      // In a real app, these would be determined by business logic
      setFeaturedProducts(memoizedAllProducts.slice(0, 4) as Product[]);
      setBestSellingProducts(memoizedAllProducts.slice(4, 8) as Product[]);
    }
  }, [memoizedAllProducts]);
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            ...
            {!isSignedIn && (
              <Button
                ...
                onClick={() => router.push('/sign-up' as any)}
              >
                Join Free
              </Button>
            )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
            {CATEGORIES.map((category, index) => (
              <Card
                ...
                onClick={() =>
                  router.push(
                    `/products?category=${category.name.toLowerCase()}` as any
                  )
                }
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {featuredProducts.map(product => (
                <Card
                  ...
                  onClick={() => router.push(`/products/${product._id}` as any)}
                >
